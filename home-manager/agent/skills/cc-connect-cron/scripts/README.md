...
bash ~/.config/opencode/skills/cc-connect-cron/scripts/cc-cron.sh add "*/30 * * * *" --prompt "检查磁盘空间并汇报" --desc "每30分钟磁盘检查" --session-mode new-per-run
...
2. `list` 列出所有任务
3. `add` 创建任务：`--cron <表达式>` 必填，其余参数透传（`--prompt` 与 `--exec` 二选一，可加 `--desc`/`--session-mode`/`--timeout-mins`/`--silent`）
4. `info` / `edit` / `exec` / `del` 管理任务
