...
## Chrome 浏览器自动化操作 (agent-browser)

通过 `agent-browser` 控制 Chrome 浏览器，使用 Chrome DevTools Protocol (CDP)。

### 启动 Chrome（Wayland / Niri）
...
# 截图保存
agent-browser --cdp 9222 screenshot /tmp/opencode/screenshot.png

# 获取当前 URL
agent-browser --cdp 9222 get url

# 点击元素 (ref 从 snapshot -i 获取)
...
# 查看是否有下载项
agent-browser --cdp 9222 snapshot -i
# 如果没有任何下载项（仅有 heading 和 searchbox），说明无下载
```
