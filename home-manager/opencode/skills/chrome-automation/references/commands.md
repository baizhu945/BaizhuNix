...
```bash
agent-browser --cdp 9222 <command> [options]
```

`agent-browser` is in `$PATH` via Nix. No `AGENT_BROWSER_HOME` or working directory setup needed.

For brevity, examples below show only the command portion.
...
```bash
agent-browser --cdp 9222 open google.com
sleep 2
agent-browser --cdp 9222 snapshot -i
...
sleep 2
agent-browser --cdp 9222 screenshot results.png
```

### Multi-page workflow
```bash
agent-browser --cdp 9222 open https://site.com
sleep 2
agent-browser --cdp 9222 snapshot -i
agent-browser --cdp 9222 click @e1  # Click link
sleep 2
agent-browser --cdp 9222 snapshot -i  # Get new refs after navigation
