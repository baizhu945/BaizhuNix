# Screen Recorder (V4-compatible local fork)

This copy is managed by Home Manager for the Noctalia v5 migration. It adds
support for the persisted V4 `screen` full-screen source and keeps the V4
HEVC/AAC/ultra/full-range defaults while retaining the upstream recorder
service. Edit this directory rather than `~/.config/noctalia`.
